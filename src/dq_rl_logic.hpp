...
#define RNG
    ...
    public:
        ...
        std::vector<int> compute_disc_indices(const std::vector<double> input_state)const;
        ...
        void print_vector (const std::vector<T1> vec)const{
            std::string sep=" ";
            std::for_each(vec.begin(),vec.end(),[this,sep] (const T1& v) { print_element(v,sep); });
            std::cout<<std::endl;}

        template<typename T1>
        void print_table (const std::vector<T1> vec,const bool lineEnd=true)const{
            std::string sep="|";
            std::for_each(vec.begin(),vec.end(),[this,sep] (const T1& v) { print_element(v,sep); });
            std::cout<<"|";
            if(lineEnd)
        ...
        template<typename T1>
        void print_element(const T1 i,const std::string sep)const {std::cout << sep << i;}
    public:
        ...
        {
          archive(CEREAL_NVP(ann)); // serialize things by passing them to the archive
        }
    };
}      // namespace
#endif //! logic defined
